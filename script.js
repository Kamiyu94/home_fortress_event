...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 【★ 動畫控制變數 ★】
    let isAnimating = false; // 是否正在閃動
    let flashTimer = null;   // 閃動的計時器
    let autoStopTimer = null; // 1.5秒自動停止的計時器
    let standbyTimer = null; // 待機閃動計時器
    ...
    // 待機閃動 (慢速)
    function startStandbyFlashing() {
        stopAllTimers(); // 先清除所有計時器
        let isGreen = true;
    ...
    function startDrawAnimation() {
        stopAllTimers();
        isAnimating = true;
        promptText.textContent = '(點擊可立即停止...)'; 
        ...
        function loop() {
            ...
            isGreen = !isGreen;
            flashTimer = setTimeout(loop, 50); // 50ms 極速閃動
        }
    ...
    function stopDrawAnimation() {
        ...
        // 1. 隨機決定結果
        const finalResult = Math.random() < 0.5 ? 'chance' : 'fate';
        const deckName = finalResult === 'chance' ? '機會' : '命運';
        const colorClass = finalResult === 'chance' ? 'green' : 'red';

        // 2. 更新 UI 顯示最終結果
        drawButtonText.textContent = deckName;
        drawButton.className = `draw-button-container ${colorClass}`;
        promptText.textContent = `抽中${deckName}！正在開啟...`;
    ...
    function forceDeckChoice(deckType) {
        ...
        drawButtonText.textContent = deckName;
        drawButton.className = `draw-button-container ${colorClass}`;
        promptText.textContent = `只剩${deckName}，正在開啟...`;
